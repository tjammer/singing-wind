...
int HashGrid::get_id(const WVec &v) const {
    int x = static_cast<int>(v.x / m_cell_size);
    int y = static_cast<int>(v.y / m_cell_size);
    return x * P1 ^ y * P2;
}

std::set<int> HashGrid::get_ids_for_object(const std::shared_ptr<ColShape> &shape) const {
    std::set<int> ids;
;
...
void HashGrid::add_object(std::shared_ptr<ColShape> shape) {
...
std::vector<std::shared_ptr<ColShape>> HashGrid::find_colliders(const std::shared_ptr<ColShape> &shape) {
...
std::vector<std::shared_ptr<ColShape>> HashGrid::get_colliders_of_point(const WVec &p) {
    std::vector<std::shared_ptr<ColShape>> colliders;
    auto id = get_id(p);
