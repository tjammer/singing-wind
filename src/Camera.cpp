...
void Camera::update(GameWorld &game_world) {
    m_center = game_world.m_pos_c[m_entity_to_follow].position;
    update();
}
...
void Camera::update() {
    auto viewport = WRenderer::get_viewport();
    m_projection = glm::ortho(-viewport.x/2.f * m_zoom, viewport.x/2.f * m_zoom, -viewport.y/2.f * m_zoom, viewport.y/2.f * m_zoom, .1f, 1.f);
    m_view = glm::lookAt(glm::vec3(m_center.x, m_center.y, -1), glm::vec3(m_center.x, m_center.y, 0), glm::vec3(0, -1, 0));
