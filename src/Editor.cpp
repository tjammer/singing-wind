...
EngineEditorState::EngineEditorState(const std::string& scene_name,
...
void
EngineEditorState::update(Engine& engine)
{
  if (!engine.get_focus()) {
    return;
  }
  ...
  if (glfwGetMouseButton(&window, GLFW_MOUSE_BUTTON_RIGHT) == GLFW_PRESS and
      not m_pressed[Menu]) {
    m_menu = !m_menu;
  ...
  if (glfwGetKey(&window, GLFW_KEY_ESCAPE) == GLFW_PRESS and
      not m_pressed[Cancel]) {
...
void
EngineEditorState::update_world()
{
  m_game_world.update_world();
}

bool
EngineEditorState::load_scene(const std::string& name)
{
...
void
EngineEditorState::save_scene(const std::string& name) const
{
  save_scene_to_fb(name, m_game_world, m_zoom);
}

inline void
new_scene(GameWorld& world)
{
...
EngineEditorState::main_menu()
{
  ...
  if (ImGui::BeginPopup("load entity")) {
    ...
    if (ImGui::Button("load entity")) {
      ...
      m_state = EditorSubState(
        new EntityMove(m_game_world, new_ent, m_camera.unproject_mouse(mpos)));
