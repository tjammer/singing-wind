...
void
add_effect(GameWorld& world, unsigned int entity, StatusEffect& effect)
{
  ...
  auto it =
    std::find_if(effects.begin(),
                 effects.end(),
                 [effect](const StatusEffect& e) { return e.id == effect.id; });
  if (it == effects.end()) {
...
void
delete_effect(GameWorld& world, unsigned int entity, StatusEffect& effect)
{
  auto& effects = world.statuseffect_c(entity).effects;
  effects.erase(std::remove_if(
    effects.begin(), effects.end(), [effect](const StatusEffect& e) {
...
void
knockback_start(GameWorld& world, unsigned int entity)
{
...
void
knockback_stop(GameWorld& world, unsigned int entity)
{
  // auto &mc = world.move_c(entity);
...
void
statuseffects::knockback_move(GameWorld& world, unsigned int entity)
{
  auto& mc = world.move_c(entity);
...
void
hitstun_tick(GameWorld& world, unsigned int entity)
{
  world.move_c(entity).time_fac *= 0.1f;
}

StatusEffect
statuseffects::hitstun()
{
  StatusEffect out;
